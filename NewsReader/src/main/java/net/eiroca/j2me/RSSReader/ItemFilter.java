...
/**
 * A class for filtering items. Items can filtered out depending on whether they are read/unread
 * and/or parsed before a deadline and/or from a certain feed.
 */
public class ItemFilter implements RecordFilter {
   ...
   * 
   * @param newdeadline the time in milliseconds since January 1, 1970 UTC the item should parsed
   *        BEFORE. (Use class fields if no deadline)
   */
  ...
  /**
   * Checks to see if the given item is a match. Returns <code>TRUE</code> if feedid is set and it
   * matches the candidate items feedid. If the deadline is set it checks that the candidate item is
