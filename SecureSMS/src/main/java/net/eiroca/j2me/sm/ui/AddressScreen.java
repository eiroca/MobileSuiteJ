...
public class AddressScreen extends Form {
  ...
  public AddressScreen(final int title, boolean isNew) {
    ...
    name = new TextField(Application.messages[SecureSMSMIDlet.MSG_NAME], "", AddressScreen.MAX_NAME_LENGTH, TextField.ANY);
    number = new TextField(Application.messages[SecureSMSMIDlet.MSG_NUMBER], "", AddressScreen.MAX_NUMBER_LENGTH, TextField.PHONENUMBER);
    password = new TextField(Application.messages[SecureSMSMIDlet.MSG_KEY], "", AddressScreen.MAX_PASSWORD_LENGTH, TextField.PASSWORD);
    append(name);
    append(number);
    append(password);
    Application.setup(this, Application.cBACK, SecureSMSMIDlet.cADRSAV);
    if (!isNew) {
